...
DAEMON_CLIENT_ID = os.getenv("DAEMON_CLIENT_ID")
DAEMON_CLIENT_SECRET = os.getenv("DAEMON_CLIENT_SECRET")

def get_graph_token(tenant_id: str) -> str:
    if not (DAEMON_CLIENT_ID and DAEMON_CLIENT_SECRET):
        raise RuntimeError("Daemon credentials missing (DAEMON_CLIENT_ID/DAEMON_CLIENT_SECRET).")
